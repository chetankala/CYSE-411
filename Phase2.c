// Chetan Kala and Konrad Stolat-Borkowski
...
#define COMMAND 0x25
#define ADD_USER 0x26  // Missing a hashtag
...
void mainLoop(FILE *logf, int sock);  // Supposed to be void instead vod
...
int readSock(int sock, char *buf, size_t len)
{
	...
	while (bytesread < len)  // Not supposed to be a semicolon
	{
...
void writeArticle(int sock, FILE *logfile, char *action)
{
	...
	size_t x, y;
	int complete = 0;
	//char *buf = malloc(1024);  // Missing a bracket
	//char *path = malloc(1024);

	char* buf  = (char*)calloc(1024, sizeof(char));
	char* path = (char*)calloc(1024, sizeof(char));

	snprintf(path, sizeof(char), "%s", &ARTICLEPATH[1]);     //strcpy(path, ARTICLEPATH); 
	snprintf(path, sizeof(char), "%s", &action[1]);          //strncat(path, &action[1], sizeof(path));

	logData(logfile, "user writing article: %s", path);

	file = fdopen(action[1], "w");   //file = fopen(&action[1], "w"); 
...
{
	FILE *file;
	char *buf = malloc(100);       //char buf[100]
	char *path = malloc(100);      //char path[100]

	logData(logfile, &action[1]);

	snprintf(path, sizeof(char), "%s", &ARTICLEPATH[1]);      // strcpy(path, ARTICLEPATH);    
	snprintf(path, sizeof(char), "%s", &action[1]);       // strcat(path, &action[1]);  

	logData(logfile, "user request to read article: %s", path);

	file = fdopen(*path, "r");   //file = fopen(path, "r"); 
	...
	{
		writeSock(sock, buf, sizeof(buf));   //writeSock(sock, buf, strlen(buf));
	}
...
{
	char* buf  = (char*)calloc(1024, sizeof(char));   //char buf[100];
	FILE *list;
	...
	list = fdopen(*"list.txt", "r");    //list = fopen("list.txt", "r");
...
// There was a duplicate function that was not supposed to be there
...
{
	char *p;
	char *buf = malloc(1024);    //char buf[1024];
...
{
	char *action = malloc(1024);   //char action[1024];
	size_t len;
	while (1)
	{
		...
		if (action[0] == ADD_USER) //ADD_USERR supposed to be ADD_USER
		{
			addUser(logfile, sock, action);	
		}
		else if (action[0] == COMMAND)   // ction supposed to be action
		{
...
int userFunctions(FILE *logfile, int sock, char *user)
{
	char* action = (char*)calloc(1024, sizeof(char));  //char action[1024];
	size_t len;
...
int authenticate(FILE *logfile, char *user, char *pass)
{
	char *search = malloc(512);  //char search[512];
	char *path = malloc(1024);   //char path[1024];
	char *userfile = malloc(1024);         //char userfile[1024];
	char *data = malloc(1024);             //char data[1024];
	FILE *file;
	int ret;
...
char *findarg(char *argbuf, char argtype)
{
	...
	while (1)
	{	
		memmove((char *)&size, ptr1, 4);  //memcpy((char *)&size, ptr1, 4);
		if (size == 0)
		{
			break;
		}	
...
void handleConnection(FILE *logfile, int sock)
{
	char *buffer = malloc(1024);  //char buffer[1024];
	char *argbuf = malloc(1024);  //char argbuf[1024]; 
	char *user = NULL;
	char *pass = NULL;
	int len = 0;
	int ret = 0;
	...
	memmove((char *)&segmentcount, ptr1, 4);    //memcpy((char *)&segmentcount, ptr1, 4); 
	...
	for (segloop = 0; segloop < segmentcount; ++segloop)
	{
		...
		memmove((char *)&segnext, ptr1, 4);
		logData(logfile, "next segment offset %i", segnext);
		ptr1 += 4;
		memmove((char *)&argsize, ptr1, 4);	
		logData(logfile, "argsize: %i", argsize);
		memmove(ptr2, ptr1, argsize);
		ptr2 += argsize;
		ptr1 += segnext;
	}
...
{
	int clientfd = 0;  //Int supposed to be lowercase
...
int main(int argc, char *argv[])
{
	...
	/* setup log file */
	logf = fopen("logfile.txt", "w");    //logf = fopen("logfile.txt", "w"); 
	...
	mainLoop(logf, sock);  // mainLoo supposed to be mainLoop	
...
void logData(FILE *logfile, char *format, ...)
{
	char *buffer = malloc(4096);   //char buffer[4096]; 
	va_list arguments;
	va_start(arguments, format);
	vsnprintf(buffer, sizeof(buffer)-1, format, arguments);  //vsnprintf(buffer, sizeof(buffer)-1, format, arguments); 
	va_end(arguments);
	fprintf(logfile, "LoggedData [Proccess:%i]: %s\n", getpid(), buffer);
	fflush(logfile);
} 
// closing bracket supposed to be at the end of function
