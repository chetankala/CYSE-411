...
void writeArticle(int sock, FILE *logfile, char *action)
{
	...
	size_t x, y;
	int complete = 0;
	char buf[1024];  // Missing a bracket
	char path[1024];
...
int authenticate(FILE *logfile, char *user, char *pass)
{
	...
	snprintf(search, sizeof(userfile)*2, "stat %s`ls %s | grep %s`", USERPATH, USERPATH, userfile);  
	ret = system(search);
	...
	snprintf(path, sizeof(userfile)*2, "%s%s", USERPATH, userfile);
